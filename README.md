# Medical Insurance Cost Prediction

## Project Overview
This project focuses on predicting the cost of medical insurance for individuals based on various features. The model aims to provide insights into factors influencing insurance pricing and assist users in understanding potential costs.

## Features
- Age
- Sex
- Body Mass Index (BMI)
- Number of Children
- Smoking Status
- Region

## Dataset Information
The dataset contains historical data about medical insurance costs, with relevant features that help in predicting costs effectively. 

## Models Used
This project implements several models to predict medical insurance costs:
- Linear Regression
- Decision Trees
- Random Forest
- XGBoost

## Usage Instructions
### Setup Steps
1. Clone the repository:
   ```bash
   git clone https://github.com/Rahat-75/Medical_Insurance_Cost_Preediction.git
   ```
2. Navigate to the project directory:
   ```bash
   cd Medical_Insurance_Cost_Preediction
   ```
3. Install the required packages:
   ```bash
   pip install -r requirements.txt
   ```

## Project Structure
```
Medical_Insurance_Cost_Preediction/
│
├── app/                   # Gradio interface for user interaction
├── models/                # Contains trained models
├── data/                  # CSV dataset
│   └── insurance_data.csv
├── notebooks/             # Jupyter notebook
│   └── analysis.ipynb
├── requirements.txt        # Python dependencies
└── main.py                # Main execution script
```

## Installation and Running Instructions
To run the project, execute the following command in the project directory:
```bash
python main.py
```

Once the app is running, it can be accessed in your web browser.